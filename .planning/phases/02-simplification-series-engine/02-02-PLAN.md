...
must_haves:
  truths:
    - "simplify(Add([3, 5, x])) produces Add([8, x]) -- numeric constants are combined"
    - "simplify(Neg(Neg(x))) returns x -- double negation is eliminated"
    - "simplify(Add([0, x])) returns x -- identity elements are cancelled"
    - "simplify(Mul([0, x])) returns Integer(0) -- zero annihilates multiplication"
    - "simplify(Add([a, Add([b, c])])) produces a flat Add([a, b, c])"
    - "Simplification terminates on adversarial inputs (50-level nested Neg, 100-wide Add)"
    - "simplify(simplify(expr)) == simplify(expr) for all tested expressions (idempotency)"
  artifacts:
...
NOTE on interaction with make_add/make_mul: The canonical constructors (make_add, make_mul) sort and dedup children. Dedup means identical ExprRefs are collapsed, which prevents the simple "count duplicate children" strategy from ever triggering after canonical construction. The rules below target a different scenario: like-term collection where the *bases* match but children are structurally distinct expressions (e.g., `Mul([2, x])` and `Mul([3, x])` are different ExprRefs but share the same base `x`). This is NOT dead code -- it handles coefficient combination, not literal deduplication.

Rules:
1. **Like-term collection in Add**: In Add(children), group terms by their "base" (strip numeric coefficient). For each child, determine if it's Mul([Integer(n), rest...]) -- then base is Mul(rest) (or rest if only one non-numeric), coefficient is n. If it's just an atom/compound, coefficient is 1. Group by base, sum coefficients. Rebuild if any groups had more than one term.
   This handles: `2*x + 3*x -> 5*x`, `x + 2*x -> 3*x`.

2. **Power collection in Mul**: In Mul(children), group terms by their "base" (strip exponent). For each child, determine if it's Pow(base, Integer(n)) -- then base is base, exponent is n. Otherwise base is child, exponent is 1. Group by base, sum exponents. Rebuild if any groups had more than one term.
   This handles: `x * x -> x^2`, `x * x^2 -> x^3`.

Note: Since make_add/make_mul sort children by ExprRef, duplicates of the exact same ExprRef are already collapsed by dedup. The collect phase only fires when terms share a base but have different numeric factors/exponents -- these are distinct ExprRefs that make_add/make_mul do NOT merge.
...
Commit: `feat(02-02): implement phased simplification engine with 4 rule phases`
  ...
  <verify>
`cargo build` succeeds with zero warnings. Additionally, add and run these inline smoke tests (either as `#[cfg(test)] mod tests` at the bottom of `simplify/mod.rs` or `rules.rs`):

1. `simplify(Integer(5))` returns the same `Integer(5)` ExprRef (atoms unchanged).
2. Construct `Neg(Neg(x))` where x is a Symbol, simplify, verify result == x.
3. Construct `Add([Integer(0), x])` where x is a Symbol, simplify, verify result == x.

These 3 smoke tests validate that the engine compiles, runs, and produces correct output for basic cases BEFORE the comprehensive test suite in Task 2. Run: `cargo test -p qsym-core simplify` passes these smoke tests.
  </verify>
  <done>
simplify/ module exists with SimplificationEngine, bottom-up traversal, and 4 rule phases (normalize, cancel, collect, simplify_arith). Code compiles cleanly. Three inline smoke tests pass: atoms unchanged, double negation eliminated, zero-identity cancelled.
  </done>
</task>
