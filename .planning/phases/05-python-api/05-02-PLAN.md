...
must_haves:
  truths:
    ...
    - "SessionInner provides get_or_create_symbol_id() helper for DSL functions"
  artifacts:
    - path: "crates/qsym-python/src/session.rs"
      provides: "QSession pyclass with frozen+Mutex pattern and SymbolId helper"
  ...
  key_links:
    ...
    - from: "crates/qsym-python/src/session.rs"
      to: "qsym_core::symbol::SymbolId"
      via: "get_or_create_symbol_id helper uses arena.symbols_mut().intern()"
      pattern: "symbols_mut\\(\\)\\.intern"
---
...
Purpose: This is the foundational API surface that all other Python functionality builds on. QSession manages the arena; QExpr wraps ExprRef with Python operator overloads (+, *, -, **) and rendering (__repr__ for REPL, _repr_latex_ for Jupyter). SessionInner also provides a `get_or_create_symbol_id()` helper that DSL functions in Plan 05-03 will use to obtain a SymbolId (NOT ExprRef) for the series variable.
...
2. Create `crates/qsym-python/src/session.rs`:
   ...
   - Add a helper method on SessionInner:
     ```rust
     impl SessionInner {
...
```
  ...
  <done>
QSession and QExpr compile. Symbols can be created, operators (+, *, -, **) produce new QExpr objects, __repr__ returns Unicode, _repr_latex_ returns LaTeX wrapped in dollar signs. SessionInner.get_or_create_symbol_id() is available for DSL functions. If Python is available, the import and basic operations work.
  </done>
</task>
...
- SessionInner.get_or_create_symbol_id() returns SymbolId (not ExprRef)
</verification>

<success_criteria>
Core QSession + QExpr API compiles and (if Python available) supports creating expressions with natural operators, rendering them as Unicode and LaTeX, and surviving GC stress without deadlock. SessionInner exposes get_or_create_symbol_id() helper that DSL functions will use in Plan 05-03.
</success_criteria>
