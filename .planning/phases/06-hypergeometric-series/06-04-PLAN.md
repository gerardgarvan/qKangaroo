...
<objective>
Implement Watson's and Bailey's transformations for very-well-poised hypergeometric series, and add Python API bindings for all Phase 6 functionality (eval_phi, eval_psi, summation formulas, transformations).

Purpose: Watson's and Bailey's transformations are the most powerful structural tools in the hypergeometric toolkit, reducing high-order series (8phi7, 10phi9) to simpler ones. Python bindings make all Phase 6 capabilities accessible to researchers, completing the phase goal.

Output: watson_transform, bailey_transform in Rust; Python DSL functions for phi/psi/summations/transformations; integration tests.
</objective>
...
```rust
#[pyfunction]
#[pyo3(signature = (session, a_num, a_den, a_pow, b_num, b_den, b_pow, c_num, c_den, c_pow, z_num, z_den, z_pow, order))]
pub fn heine1(session: &QSession, /* 12 param triples + order */) -> Option<(QSeries, QSeries)>
```

This is many parameters. Simplify the API:
