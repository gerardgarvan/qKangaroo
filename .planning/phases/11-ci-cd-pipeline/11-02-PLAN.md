...
**Important note on wheel output path:** maturin-action with `working-directory` may place wheels at the workspace root `target/wheels/` instead of `crates/qsym-python/target/wheels/`. To handle this robustly, use a glob pattern for upload: `path` should try `target/wheels/*.whl` first. If during testing the path is wrong, adjust. A safe approach: after the maturin build step, add a `find . -name "*.whl" -path "*/target/wheels/*"` debug step (can be removed later), or use `path: target/wheels/*.whl` at workspace root level since maturin-action runs from the repo root and sets working-directory internally.

Actually, based on the research, use `path: crates/qsym-python/target/wheels/*.whl` first. The maturin-action `working-directory` input changes where maturin runs, but the target directory follows the Cargo workspace root. Since there is a workspace Cargo.toml at the repo root, the target directory is likely `./target/wheels/`. So use `path: target/wheels/*.whl` for the upload path.
...
Do NOT include the windows-wheels job yet -- that will be added in Task 2. Leave `needs: [linux-wheels, windows-wheels, sdist]` in the publish job so it references the job that Task 2 will add. This ensures the file is built incrementally.

Wait -- that would fail validation if windows-wheels job does not exist. Instead, write the COMPLETE file in this task including windows-wheels as a placeholder comment, then Task 2 fills it in. Actually, let me restructure: write the entire file in ONE task since it is a single workflow file. Then use Task 2 for a different concern.

REVISED APPROACH: Write the complete release.yml in this single task, including ALL four jobs (linux-wheels, windows-wheels, sdist, publish). This is one file and all four jobs are tightly coupled through the publish job's `needs` list.

**Job 3 (windows-wheels) -- add here, not in separate task:**
