...
2. **Find LHS recurrence.** Call `detect_n_params` on the LHS series to identify n-dependent parameters. Then call `q_zeilberger(&lhs_series, q_val, n_test, max_order, variable, truncation_order)`. If result is NoRecurrence, return Failed.
...
1. `test_prove_q_gauss` -- The q-Gauss summation formula: 2phi1(a, b; c; q, c/(ab)) = (c/a;q)_inf*(c/b;q)_inf / ((c;q)_inf*(c/(ab);q)_inf). Specialize a -> q^{-n} to make it terminating. LHS builder: 2phi1(q^{-n}, b; c; q, cq^n/b) (terminating). RHS builder: evaluate the product formula as FPS. Use q=1/2, b=q^2, c=q^3 (concrete values). Prove the identity.
