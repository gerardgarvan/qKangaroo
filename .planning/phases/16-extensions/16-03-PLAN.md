...
```rust
/// Normalize a HypergeometricSeries for comparison and hashing.
/// Returns a canonical representation as a sorted tuple of (coeff_numer, coeff_denom, power)
/// for upper and lower params, plus argument.
///
/// Two series are "structurally equal" if they have the same (r,s), same sorted
/// parameter multisets, and same argument.
fn normalize_series_key(series: &HypergeometricSeries) -> Vec<(String,)>
```

Implementation: For each upper param QMonomial, produce a canonical string "numer/denom:power" from coeff and power. Sort the upper param strings. Do the same for lower params. Combine into a single Vec or String key. This allows using a HashSet<String> for visited detection.

SIMPLER approach: Since QMonomial contains QRat (which has exact comparison via PartialEq), sort upper and lower params by (power, coeff) and concatenate their Debug representations into a single String key. Use this for the HashSet.
   ...
   d. For each (name, transform_fn) in transforms:
      ...
      - If Some(result):
        ...
        - Check if result.transformed matches target:
          Compute eval_phi(&result.transformed) and compare with eval_phi(target) after accounting for prefactor: does cumulative_prefactor * eval_phi(result.transformed) == eval_phi(target)? Wait -- the target is a SERIES, and we want the transformed source * prefactor to equal the target (no prefactor). So: eval_phi(source) == total_prefactor * eval_phi(target). Actually, the chain transforms source into (total_prefactor * new_series). We want total_prefactor * eval_phi(new_series) == eval_phi(source). But the user wants to find: source ~ target. Meaning eval_phi(source) == total_prefactor * eval_phi(chain_end).

          ACTUALLY: Each transformation gives result.transformed and result.prefactor such that eval_phi(input) == result.prefactor * eval_phi(result.transformed). So after a chain of transformations: eval_phi(source) == total_prefactor * eval_phi(chain_end). We want chain_end to MATCH target. The match condition is: eval_phi(chain_end) == eval_phi(target). This means eval_phi(source) == total_prefactor * eval_phi(target).
