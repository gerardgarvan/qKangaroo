...
must_haves:
  ...
  key_links:
    - from: "docs/examples/maple_migration.ipynb"
      ...
      pattern: "aqprod|etaq|theta|partition|prodmake|findlincombo|phi|prove|mock|q_gosper|q_zeilberger|q_petkovsek|prove_nonterminating"
...
- Markdown: Table:
  ...
  | *batch processing* | `batch_generate(s, generators, params, order)` | Generate multiple series |
...
- Markdown: Table:
  ...
  | `findlincombo_modp(...)` | `findlincombo_modp(series_list, prime, order)` | Linear relations mod p |
  | `findhom_modp(...)` | `findhom_modp(series_list, degree, prime, order)` | Poly relations mod p |
- Code cell: `findcong(partition_gf(s, 100), [5, 7, 11])` -- Ramanujan congruences. Output: the three congruences.
- Code cell: `findlincombo_modp([partition_gf(s, 50), odd_parts_gf(s, 50)], 5, 50)` -- search for a relation mod 5. Show result.
...
- Markdown: Table of mock theta functions:
  | Maple | q-Kangaroo | Notes |
  |-------|------------|-------|
  | *third order* | `f3(s, N)`, `phi3(s, N)`, `psi3(s, N)`, `chi3(s, N)`, `omega3(s, N)`, `nu3(s, N)`, `rho3(s, N)` | 7 third-order |
  | *fifth order* | `f05(s, N)`, `phi05(s, N)`, `psi05(s, N)`, `chi05(s, N)`, `F05(s, N)`, `f15(s, N)`, `phi15(s, N)`, `psi15(s, N)`, `chi15(s, N)`, `F15(s, N)` | 10 fifth-order |
  | *seventh order* | `F07(s, N)`, `F17(s, N)`, `F27(s, N)` | 3 seventh-order |
  | Appell-Lerch | `appell_lerch_bilateral(s, ...)`, `appell_lerch_g2(s, ...)`, `appell_lerch_g3(s, ...)` | Bilateral sums |
  | Bailey pairs | `bailey_pair(pair_name)` | "unit", "rogers_ramanujan", "q_binomial" |
  | Bailey lemma | `bailey_lemma(s, pair, a_num, a_den, a_pow, order)` | Apply Bailey's lemma |
...
- Code cell: `f3(s, 20)` -- third-order mock theta $f(q)$. Output: `1 + q + q^2 + q^3 + 2*q^4 + ...`. Also `F07(s, 15)` for seventh-order.
- Code cell: `bailey_pair("rogers_ramanujan")` -- show the R-R pair alpha/beta series definitions.
- Code cell: `bailey_chain(s, "rogers_ramanujan", 1, 1, 1, 1, 0, 2, 20)` -- one step of Bailey chain from R-R pair.
- Markdown: Note: The 20 classical mock theta functions are named by order and variant. Garvan's Maple packages do not include mock theta functions -- this is a q-Kangaroo extension.
...
**14. Group 13: Extensions** (3 cells, 3 operations)
- Markdown: Table:
  | Maple | q-Kangaroo | Notes |
  |-------|------------|-------|
  ...
  | *no equiv* | `prove_identity_qratio(s, ...)` | Ratio-based identity proof |
...
- Markdown: Common patterns: (1) All functions take `QSession` as first arg, (2) Monomials are `(num, den, power)` tuples, (3) `None` for infinity in Pochhammer, (4) Results are QSeries or dicts.
...
7. Maple syntax shown alongside Python equivalent in tables
  ...
  <done>
maple_migration.ipynb exists with ~50-60 cells covering all 13 function groups with Maple -> Python translation tables, at least 30 operations demonstrated with pre-computed outputs, and the "no Maple equivalent" extensions (mock theta, q-Gosper, algorithmic proving) clearly marked.
  </done>
</task>
