...
3. Implement `format_bivariate(bs: &BivariateSeries, symbols: &SymbolRegistry) -> String`:
   ...
   - For each `(z_exp, fps)`:
     - Format the FPS coefficient as a string using the existing `format_series` function (make it `pub(crate)` if it's currently private)
     - Format the z-power part:
       - z_exp == 0: just the fps_str (no z variable shown)
       - z_exp == 1: `{outer_var}`
       - z_exp == -1: `{outer_var}^(-1)`
       - z_exp > 1: `{outer_var}^{z_exp}`
       - z_exp < -1: `{outer_var}^({z_exp})`
     - Determine if the FPS is "simple" (a single monomial term). If so, format compactly: e.g., `q^3*z^2` instead of `(q^3 + O(q^T))*z^2`. Specifically:
       - If fps has exactly 1 nonzero term AND truncation_order >= POLYNOMIAL_ORDER (i.e., it's effectively a polynomial term), format as `coeff_str*z_part` without parentheses
       - Otherwise, format as `(fps_str)*z_part`
     - Actually, for bivariate series from tripleprod etc., the FPS will always have a truncation order (NOT POLYNOMIAL_ORDER). So for clarity: if the FPS has only 1 nonzero term, display as `coeff_monomial*z_part` (no parens). If it has 0 or 2+ terms, display as `(fps_str)*z_part`.
     - For the z_exp == 0 case: just display `fps_str` if it's the only term, or `(fps_str)` wrapped in parens if there are other z-exponent terms (for clarity). Actually simpler: always just show the fps_str for z^0 without a z part.
   - Handle signs: track whether this is the first term. For subsequent terms, if the FPS coefficient is a single negative term, use ` - ` separator. For multi-term FPS or positive single-term, use ` + `. For simplicity, just use ` + ` always and let the FPS string handle its own sign (the format_series already starts with `-` for negative leading terms).
   - If all terms are empty (zero bivariate), display as `O({inner_var}^{trunc})`.
   - Append ` + O({inner_var}^{trunc})` at the end (since the inner series are truncated).

   **Recommended simpler display approach:** For each z^k term in descending order, format as:
   - Single-term FPS (1 nonzero coeff): display inline like `q^3*z^2` or `-q*z^(-1)`
   - Multi-term FPS: display as `(fps_str)*z^k`
   - z^0 term: just `fps_str` or `(fps_str)`
