...
<objective>
Fix aqprod 3-arg truncation bug and add Garvan 2-arg signatures for theta2/3/4.

Purpose: FIX-01 fixes aqprod(q,q,n) to return the full finite polynomial instead of truncating to O(q^n). FIX-02 adds theta3(q,T) 2-arg Garvan convention so users don't need the legacy theta3(T) form.
Output: Modified eval.rs with correct aqprod dispatch and multi-arity theta dispatch, updated help.rs signatures.
</objective>
...
<task type="auto">
  <name>Task 1: Fix aqprod 3-arg truncation and add theta 2-arg dispatch</name>
...
Replace each of the three theta function dispatches. Currently each uses `expect_args(name, args, 1)`. Change to multi-arity dispatch supporting 1-arg (legacy) and 2-arg (Garvan) forms.
...
- `"theta2" => "(order)"` to `"theta2" => "(T) or (q, T)"`
- `"theta3" => "(order)"` to `"theta3" => "(T) or (q, T)"`
- `"theta4" => "(order)"` to `"theta4" => "(T) or (q, T)"`
...
4. `dispatch_theta2_2arg`: Call `theta2` with Symbol("q"), Integer(10). Assert result is Series (not error).

5. `dispatch_theta4_2arg`: Call `theta4` with Symbol("q"), Integer(10). Assert result is Series (not error).

6. `dispatch_theta3_1arg_unchanged`: Call `theta3` with Integer(20). Assert still works (regression test -- should already pass but good to have).
  </action>
  <verify>
Run: `export PATH="/c/mingw64-gcc/mingw64/bin:/c/cygwin64/bin:/c/Users/Owner/.cargo/bin:$PATH" && cargo test -p qsym-cli dispatch_aqprod_maple_3arg_polynomial_order dispatch_aqprod_4arg_unchanged dispatch_theta3_2arg dispatch_theta2_2arg dispatch_theta4_2arg dispatch_theta3_1arg_unchanged -- --nocapture`
All 6 new tests pass.
  </verify>
  <done>aqprod(q,q,5) returns exact polynomial with POLYNOMIAL_ORDER sentinel; theta2/3/4 accept both 1-arg and 2-arg forms; all tests pass.</done>
</task>
...
Change `signature` from `"theta2(order)"` to `"theta2(T) or theta2(q, T)"`.
Update `description` to mention the 2-arg Garvan form.

**Update theta3 help entry** (help.rs line ~278):
Change `signature` from `"theta3(order)"` to `"theta3(T) or theta3(q, T)"`.
Update `description` to mention the 2-arg Garvan form.

**Update theta4 help entry** (help.rs line ~285):
Change `signature` from `"theta4(order)"` to `"theta4(T) or theta4(q, T)"`.
Update `description` to mention the 2-arg Garvan form.
...
All help tests pass.
  ...
  <done>Help entries for aqprod and theta2/3/4 reflect the new argument forms.</done>
</task>
...
- theta2(q,T) and theta4(q,T) both accepted
- All 763+ CLI tests pass plus 6 new tests
</success_criteria>
