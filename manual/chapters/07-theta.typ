...
q-Kangaroo implements the three "one-variable" Jacobi theta functions in
the $q$-notation convention. Each is expressible both as a sum over
integers and as an infinite product via the Jacobi triple product identity.
...
#func-entry(
  ...
  math-def: [
    ...
    *Product form:*
    ...
    Equivalently, $theta_4(q) = J(1, 2) = $ `jacprod(1, 2, order)`.
  ],
  params: (
    ([order], [Integer], [Truncation order for the result]),
  ),
